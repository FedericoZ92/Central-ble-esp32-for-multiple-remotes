...
// Application Callback. Called when the custom subscribable chatacteristic in the remote GATT server is read.
...
static int blecent_on_custom_write(uint16_t conn_handle,
                                    const struct ble_gatt_error *error,
                                    struct ble_gatt_attr *attr,
                                    void *arg)
{
...
static int blecent_on_custom_subscribe(uint16_t conn_handle,
                            const struct ble_gatt_error *error,
                            struct ble_gatt_attr *attr,
                            void *arg)
{
                ...
                void *arg)
{
    ...
    // Write two bytes (99, 100) to the alert-notification-control-point characteristic.
    const struct peer_chr *chr;
    uint8_t value[2];
    int rc;
    const struct peer *peer = peer_find(conn_handle);
...
static int ext_blecent_should_connect(const struct ble_gap_ext_disc_desc *disc)
{
    ...
    // The device has to advertise support for the Alert Notification service (0x1811).
    do {
...
// Connects to the sender of the specified advertisement of it looks interesting.  
// A device is "interesting" if it advertises connectability and support for the Alert Notification service.
...
// The application associates a GAP event callback with each connection that is established.  
// blecent uses the same callback for all connections.
...
void task_main(void *pvParameters) 
{
    ...
    //***********Gpios***********    
    GpioManager gpioManager = GpioManager();

    gpioManager.configure();

    ESP_LOGI(MAIN_TAG, "INIT MAIN......................\n");

    //// ***********Buttons***********
    Button onOffButton = Button(ON_SW_BUTTON, LONG_ONOFF_BUTTON_PERIOD, SHORT_BUTTON_PERIOD, HIGH_LEVEL, ButtonType::HARDWARE); 
    ButtonTask onOffButtonMonitoringTask = ButtonTask("button task on-off", &onOffButton);
    GpioHandler gpioOnOffHandler(ON_SW_BUTTON, &onOffButtonMonitoringTask);
    //
    V(Button scrollButton = Button(SCROLL_BUTTON_GPIO, LONG_SCROLL_BUTTON_PERIOD, SHORT_BUTTON_PERIOD, HIGH_LEVEL, ButtonType::SCROLL));
    V(ButtonTask scrollButtonMonitoringTask = ButtonTask("button task scroll", &scrollButton));
    V(GpioHandler gpioScrollHandler(SCROLL_BUTTON_GPIO, &scrollButtonMonitoringTask));
    //
    KeyboardManager keyboardManager = KeyboardManager(VT(&scrollButton,nullptr), VT(&scrollButtonMonitoringTask,nullptr), &onOffButton, &onOffButtonMonitoringTask);
    keyboardManager.attachKeyboardListener(&modeManager, PRIORITY_AVERAGE);
    V(keyboardManager.attachKeyboardListener(&screenManager, PRIORITY_HIGH)); 
    keyboardManager.attachKeyboardListener(&bed, PRIORITY_VERY_HIGH);

    ESP_LOGI(MAIN_TAG, "bluetooth init");
    bluetooth.init();
    Ble_Start();
 
    ESP_LOGI(MAIN_TAG, "on-off handler init");
    gpioOnOffHandler.init(); // set up the interrupt
    V(ESP_LOGI(MAIN_TAG, "scroll handler init"));
    V(gpioScrollHandler.init());  // set up the interrupt
    ESP_LOGI(MAIN_TAG, "keyboard button tasks init");
    V(keyboardManager.init());
    ...
    while(true){
        ESP_LOGW(SCHED_TAG, "---Good morning! Mode is %s---", modeTypeToString(modeManager.getWorkingMode()).c_str());   
        vTaskDelay(pdMS_TO_TICKS(1000));
        checkMemory();
    }
    ESP_LOGI(MAIN_TAG, "leaving loop......");
    vTaskDelete(NULL);
    return;  
}

// far chiamare un task (xtask create, allocare tutto e metter il while true lì dentro)
// Così è più chiara la priorità del thread chiamato dal main
extern "C" void app_main(void) 
{  
    xTaskCreate(task_main, "main task", CONFIG_ESP_MAIN_TASK_STACK_SIZE, NULL, 7, NULL); //16384 size
}
