...
#define SSD1305_SET_AREA_COLOR_MODE	                    0xD8
...
#define SSD1305_row_data(Data, size)    I2C_Write_Bytes(Data, size, SSD1305_DATA_CONTINUE)
...
public:
	SSD1305(int16_t oledWidth = SCREEN_WIDTH, int16_t oledHeight = SCREEN_HEIGHT);
	~SSD1305();
	...
	void OLEDFillScreen(uint8_t pixel, uint8_t msDelay);
	void OLEDFillPage(uint8_t page_num, uint8_t pixels,uint8_t msDelay);
...
protected:
	virtual void OLED_I2C_OFF(void) = 0; 
	virtual void OLED_I2C_ON(void) = 0; 

	virtual void I2C_Write_Byte(uint8_t value, uint8_t Cmd) = 0;
	virtual void I2C_Write_Bytes(const uint8_t* value, uint8_t size, uint8_t Cmd) = 0;
...
private:
	...
	bool initializedByCommands = false;
	int16_t OLEDwidth;
	int16_t OLEDheight;
	int8_t OLEDpageNumber;
	uint8_t bufferWidth;
	uint8_t bufferHeight;
	uint8_t screenBuffer[SCREEN_WIDTH * (SCREEN_HEIGHT/8)+1];
